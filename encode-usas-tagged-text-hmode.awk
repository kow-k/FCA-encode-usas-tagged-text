...
function get_encoding(x) {
	# A
	if (x ~ /.+[_\/]+A1[^0-9]*/) { A1++; A1L[A1] = x }
	if (x ~ /.+[_\/]+A2[^0-9]*/) { A2++; A2L[A2] = x }
	if (x ~ /.+[_\/]+A3[^0-9]*/) { A3++; A3L[A3] = x }
	if (x ~ /.+[_\/]+A4[^0-9]*/) { A4++; A4L[A4] = x }
	if (x ~ /.+[_\/]+A5[^0-9]*/) { A5++; A5L[A5] = x }
	if (x ~ /.+[_\/]+A6[^0-9]*/) { A6++; A6L[A6] = x }
	if (x ~ /.+[_\/]+A7[^0-9]*/) { A7++; A7L[A7] = x }
	if (x ~ /.+[_\/]+A8[^0-9]*/) { A8++; A8L[A8] = x }
	if (x ~ /.+[_\/]+A9[^0-9]*/) { A9++; A9L[A9] = x }
	if (x ~ /.+[_\/]+A10[^0-9]*/) { A10++; A10L[A10] = x }
	if (x ~ /.+[_\/]+A11[^0-9]*/) { A11++; A11L[A11] = x }
	if (x ~ /.+[_\/]+A12[^0-9]*/) { A12++; A12L[A12] = x }
	if (x ~ /.+[_\/]+A13[^0-9]*/) { A13++; A13L[A13] = x }
	if (x ~ /.+[_\/]+A14[^0-9]*/) { A14++; A14L[A14] = x }
	if (x ~ /.+[_\/]+A15[^0-9]*/) { A15++; A15L[A15] = x }
	# B
	if (x ~ /.+[_\/]+B1[^0-9]*/) { B1++; B1L[B1] = x }
	if (x ~ /.+[_\/]+B2[^0-9]*/) { B2++; B2L[B2] = x }
	if (x ~ /.+[_\/]+B3[^0-9]*/) { B3++; B3L[B3] = x }
	if (x ~ /.+[_\/]+B4[^0-9]*/) { B4++; B4L[B4] = x }
	if (x ~ /.+[_\/]+B5[^0-9]*/) { B5++; B5L[B5] = x }
	# C
	if (x ~ /.+[_\/]+C1[^0-9]*/) { C1++; C1L[C1] = x }
	# E
	if (x ~ /.+[_\/]+E1[^0-9]*/) { E1++; E1L[E1] = x }
	if (x ~ /.+[_\/]+E2[^0-9]*/) { E2++; E2L[E2] = x }
	if (x ~ /.+[_\/]+E3[^0-9]*/) { E3++; E3L[E3] = x }
	if (x ~ /.+[_\/]+E4[^0-9]*/) { E4++; E4L[E4] = x }
	if (x ~ /.+[_\/]+E5[^0-9]*/) { E5++; E5L[E5] = x }
	if (x ~ /.+[_\/]+E6[^0-9]*/) { E6++; E6L[E6] = x }
	# F
	if (x ~ /.+[_\/]+F1[^0-9]*/) { F1++; F1L[F1] = x }
	if (x ~ /.+[_\/]+F2[^0-9]*/) { F2++; F2L[F2] = x }
	if (x ~ /.+[_\/]+F3[^0-9]*/) { F3++; F3L[F3] = x }
	if (x ~ /.+[_\/]+F4[^0-9]*/) { F4++; F4L[F4] = x }
	# G
	if (x ~ /.+[_\/]+G1[^0-9]*/) { G1++; G1L[G1] = x }
	if (x ~ /.+[_\/]+G2[^0-9]*/) { G2++; G2L[G2] = x }
	if (x ~ /.+[_\/]+G3[^0-9]*/) { G3++; G3L[G3] = x }
	# H
	if (x ~ /.+[_\/]+H1[^0-9]*/) { H1++; H1L[H1] = x }
	if (x ~ /.+[_\/]+H2[^0-9]*/) { H2++; H2L[H2] = x }
	if (x ~ /.+[_\/]+H3[^0-9]*/) { H3++; H3L[H3] = x }
	if (x ~ /.+[_\/]+H4[^0-9]*/) { H4++; H4L[H4] = x }
	if (x ~ /.+[_\/]+H5[^0-9]*/) { H5++; H5L[H5] = x }
	# I
	if (x ~ /.+[_\/]+I1[^0-9]*/) { I1++; I1L[I1] = x }
	if (x ~ /.+[_\/]+I2[^0-9]*/) { I2++; I2L[I2] = x }
	if (x ~ /.+[_\/]+I3[^0-9]*/) { I3++; I3L[I3] = x }
	if (x ~ /.+[_\/]+I4[^0-9]*/) { I4++; I4L[I4] = x }
	# K
	if (x ~ /.+[_\/]+K1[^0-9]*/) { K1++; K1L[K1] = x }
	if (x ~ /.+[_\/]+K2[^0-9]*/) { K2++; K2L[K2] = x }
	if (x ~ /.+[_\/]+K3[^0-9]*/) { K3++; K3L[K3] = x }
	if (x ~ /.+[_\/]+K4[^0-9]*/) { K4++; K4L[K4] = x }
	if (x ~ /.+[_\/]+K5[^0-9]*/) { K5++; K5L[K5] = x }
	if (x ~ /.+[_\/]+K6[^0-9]*/) { K6++; K6L[K6] = x }
	# L
	if (x ~ /.+[_\/]L1[^0-9]*/) { L1++; L1L[L1] = x }
	if (x ~ /.+[_\/]L2[^0-9]*/) { L2++; L2L[L2] = x }
	if (x ~ /.+[_\/]L3[^0-9]*/) { L3++; L3L[L3] = x }
	# M
	if (x ~ /.+[_\/]M1[^0-9]*/) { M1++; M1L[M1] = x }
	if (x ~ /.+[_\/]M2[^0-9]*/) { M2++; M2L[M2] = x }
	if (x ~ /.+[_\/]M3[^0-9]*/) { M3++; M3L[M3] = x }
	if (x ~ /.+[_\/]M4[^0-9]*/) { M4++; M4L[M4] = x }
	if (x ~ /.+[_\/]M5[^0-9]*/) { M5++; M5L[M5] = x }
	if (x ~ /.+[_\/]M6[^0-9]*/) { M6++; M6L[M6] = x }
	if (x ~ /.+[_\/]M7[^0-9]*/) { M7++; M7L[M7] = x }
	if (x ~ /.+[_\/]M8[^0-9]*/) { M8++; M8L[M8] = x }
	# N
	if (x ~ /.+[_\/]N1[^0-9]*/) { N1++; N1L[N1] = x }
	if (x ~ /.+[_\/]N2[^0-9]*/) { N2++; N2L[N2] = x }
	if (x ~ /.+[_\/]N3[^0-9]*/) { N3++; N3L[N3] = x }
	if (x ~ /.+[_\/]N4[^0-9]*/) { N4++; N4L[N4] = x }
	if (x ~ /.+[_\/]N5[^0-9]*/) { N5++; N5L[N5] = x }
	if (x ~ /.+[_\/]N6[^0-9]*/) { N6++; N6L[N6] = x }
	# O
	if (x ~ /.+[_\/]O1[^0-9]*/) { O1++; O1L[O1] = x }
	if (x ~ /.+[_\/]O2[^0-9]*/) { O2++; O2L[O2] = x }
	if (x ~ /.+[_\/]O3[^0-9]*/) { O3++; O3L[O3] = x }
	if (x ~ /.+[_\/]O4[^0-9]*/) { O4++; O4L[O4] = x }
	# P
	if (x ~ /.+[_\/]P1[^0-9]*/) { P1++; P1L[P1] = x }
	# Q
	if (x ~ /.+[_\/]Q1[^0-9]*/) { Q1++; Q1L[Q1] = x }
	if (x ~ /.+[_\/]Q2[^0-9]*/) { Q2++; Q2L[Q2] = x }
	if (x ~ /.+[_\/]Q3[^0-9]*/) { Q3++; Q3L[Q3] = x }
	if (x ~ /.+[_\/]Q4[^0-9]*/) { Q4++; Q4L[Q4] = x }
	# S
	if (x ~ /.+[_\/]S1[^0-9]*/) { S1++; S1L[S1] = x }
	if (x ~ /.+[_\/]S2[^0-9]*/) { S2++; S2L[S2] = x }
	if (x ~ /.+[_\/]S3[^0-9]*/) { S3++; S3L[S3] = x }
	if (x ~ /.+[_\/]S4[^0-9]*/) { S4++; S4L[S4] = x }
	if (x ~ /.+[_\/]S5[^0-9]*/) { S5++; S5L[S5] = x }
	if (x ~ /.+[_\/]S6[^0-9]*/) { S6++; S6L[S6] = x }
	if (x ~ /.+[_\/]S7[^0-9]*/) { S7++; S7L[S7] = x }
	if (x ~ /.+[_\/]S8[^0-9]*/) { S8++; S8L[S8] = x }
	if (x ~ /.+[_\/]S9[^0-9]*/) { S9++; S9L[S9] = x }
	# T
	if (x ~ /.+[_\/]T1[^0-9]*/) { T1++; T1L[T1] = x }
	if (x ~ /.+[_\/]T2[^0-9]*/) { T2++; T2L[T2] = x }
	if (x ~ /.+[_\/]T3[^0-9]*/) { T3++; T3L[T3] = x }
	if (x ~ /.+[_\/]T4[^0-9]*/) { T4++; T4L[T4] = x }
	# W
	if (x ~ /.+[_\/]W1[^0-9]*/) { W1++; W1L[W1] = x }
	if (x ~ /.+[_\/]W2[^0-9]*/) { W2++; W2L[W2] = x }
	if (x ~ /.+[_\/]W3[^0-9]*/) { W3++; W3L[W3] = x }
	if (x ~ /.+[_\/]W4[^0-9]*/) { W4++; W4L[W4] = x }
	if (x ~ /.+[_\/]W5[^0-9]*/) { W5++; W5L[W5] = x }
	# X
	if (x ~ /.+[_\/]X1[^0-9]*/) { X1++; X1L[X1] = x }
	if (x ~ /.+[_\/]X2[^0-9]*/) { X2++; X2L[X2] = x }
	if (x ~ /.+[_\/]X3[^0-9]*/) { X3++; X3L[X3] = x }
	if (x ~ /.+[_\/]X4[^0-9]*/) { X4++; X4L[X4] = x }
	if (x ~ /.+[_\/]X5[^0-9]*/) { X5++; X5L[X5] = x }
	if (x ~ /.+[_\/]X6[^0-9]*/) { X6++; X6L[X6] = x }
	if (x ~ /.+[_\/]X7[^0-9]*/) { X7++; X7L[X7] = x }
	if (x ~ /.+[_\/]X8[^0-9]*/) { X8++; X8L[X8] = x }
	if (x ~ /.+[_\/]X9[^0-9]*/) { X9++; X9L[X9] = x }
	# Y
	if (x ~ /.+[_\/]Y1[^0-9]*/) { Y1++; Y1L[Y1] = x }
	if (x ~ /.+[_\/]Y2[^0-9]*/) { Y2++; Y2L[Y2] = x }
	# Z
	if (x ~ /.+[_\/]Z1[^0-9]*/)  { Z1++ ; Z1L[Z1] = x }
	if (x ~ /.+[_\/]Z2[^0-9]*/)  { Z2++ ; Z2L[Z2] = x }
	if (x ~ /.+[_\/]Z3[^0-9]*/)  { Z3++ ; Z3L[Z3] = x }
	if (x ~ /.+[_\/]Z4[^0-9]*/)  { Z4++ ; Z4L[Z4] = x }
	if (x ~ /.+[_\/]Z5[^0-9]*/)  { Z5++ ; Z5L[Z5] = x }
	if (x ~ /.+[_\/]Z6[^0-9]*/)  { Z6++ ; Z6L[Z6] = x }
	if (x ~ /.+[_\/]Z7[^0-9]*/)  { Z7++ ; Z7L[Z7] = x }
	if (x ~ /.+[_\/]Z8[^0-9]*/)  { Z8++ ; Z8L[Z8] = x }
	if (x ~ /.+[_\/]Z9[^0-9]*/)  { Z9++ ; Z9L[Z9] = x }
	if (x ~ /.+[_\/]Z99[^0-9]*/) { Z99++ ; Z99L[Z99] = x }
}
